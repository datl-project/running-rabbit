...
func rect_of_carrot_at(_x,_y) -> Rect2:
	var x = Carrot.rect.position.x + _x
	var y = Carrot.rect.position.y + _y
	return Rect2(Vector2(x,y),Carrot.rect.size)
...
#		var _y = rect.position.x + randi_range(0,rect.size.y)
		...
		var _rect  = rect_of_carrot_at(_x,_y)
...
func generate():
	...
	for i in range(generating_number):
		...
		if pos != null:
			$"..".rect_occupied.push_back(rect_of_carrot_at(pos.x,pos.y))
